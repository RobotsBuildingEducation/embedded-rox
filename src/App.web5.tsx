...
/**
 * looks for a record with embedded-rox.app and if the set of records does not contain it, it gets created.
 */
export const createWebNodeRecord = async (web5, set, userUnlocks) => {
  let robots = set.find((item) =>
    item?.data?.protocol?.includes("https://embedded-rox.app")
  );
    ...
    message: {
      filter: {
        recordId: dwnRecords?.find(
          (item) => item?.data?.protocol === "https://embedded-rox.app"
        )?.id,
      },
    },
  });
...
export const useSharedNostr = (initialNpub, initialNsec) => {
  ...
  const generateNostrKeys = async (userDisplayName = null) => {
    ...
    //       about: "A student onboarded with Rox",
    //     }),
    //     0,
    //     publicKey,
    //     encodedNsec
